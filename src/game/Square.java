...
public enum Square {
    ...
    private static final HashMap<Map.Entry<Integer, Integer>, Square> squareLookup = Arrays.stream(Square.values())
        .collect(
            HashMap::new,
            (HashMap<Map.Entry<Integer, Integer>, Square> map, Square squareValue)
                    -> map.put(Map.entry(squareValue.column, squareValue.row), squareValue),
            HashMap::putAll
        );
